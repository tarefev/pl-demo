...
function mwRunStep() {
  ...
  const el = addMessage('assistant', '');
  el.classList.add('mw-step-msg');
  // заголовок шага — над блоком; кнопки «Назад/Далее» рисует стенд (demo-flow.js)
  el.innerHTML = `
    <div class="step-head">Шаг ${done + 1} из ${total}</div>
  ...
  // стенд вариантов навигации: регистрируем шаг и рисуем «Назад/Далее»
  if (typeof flowStepMounted === 'function') flowStepMounted(el, done + 1, total);
  scrollFeed();
}
...
function mwChoice(el, s) {
  ...
  // на стенде выбор только подсвечивает вариант — дальше ведёт кнопка «Далее»
  let picked = null;
  opts.forEach(o => {
    ...
    b.addEventListener('click', () => {
      ...
      box.querySelectorAll('button').forEach(x => x.classList.remove('is-picked'));
  ...
  ok.addEventListener('click', () => {
    if (state.busy) return;
    if (!picked) return mwWarn(el, 'Выберите один из вариантов.');
    el.querySelectorAll('.mw-choice').forEach(x => x.disabled = true);
    ok.disabled = true;
...
function mwPreview() {
  ...
  // стенд навигации: после последнего шага кнопки исчезают — только чеклист
  mwFinish(built);
  if (typeof flowFinish === 'function') flowFinish();
  scrollFeed();
}
